...
class TABTest(unittest.TestCase):
    def setUp(self):
        self.tab = TAB(EXAMPLE1_DIR)
        if os.path.exists(self.tab.current_file):
            os.remove(self.tab.current_file)
    ...
    def test_last_file(self):
        assert self.tab.last_file.endswith('2017-04-01-logbook.yaml')

    def test_load_file_that_does_not_exist(self):
        content = self.tab.load_file(os.path.join(EXAMPLE1_DIR, 'fake'))
        assert content == dict(TODO=[], Accomplished=[], Backlog=[])

    def test_load_file(self):
        filename = os.path.join(EXAMPLE1_DIR, '2017-01-01-logbook.yaml')
        content = self.tab.load_file(filename)
    ...
    def test_create_file_for_today(self):
        self.tab.create_today_file()
        assert os.path.exists(self.tab.current_file)
...
class BacklogTest(unittest.TestCase):
    ...
    def test_no_date_wont_change_anything(self):
        ...
        with open(filename, 'w+') as fd:
            yaml.dump(data, fd)
        self.tab.create_today_file()
        content = self.tab.load_file(self.tab.current_file)
